...
/// ```
#[repr(C)]
pub struct RvState<X: Xlen, T: TracerState = (), const NUM_REGS: usize = NUM_REGS_I> {
    ...
    /// Target instret for suspension (suspend when instret >= target).
...
impl<X: Xlen, T: TracerState, const NUM_REGS: usize> RvState<X, T, NUM_REGS> {
...
impl<X: Xlen, T: TracerState, const NUM_REGS: usize> Default for RvState<X, T, NUM_REGS> {
    fn default() -> Self {
        Self::new()
    }
}

/// Type alias for RV32I state (32-bit, 32 registers, no tracer).
pub type Rv32State = RvState<rvr_ir::Rv32, (), NUM_REGS_I>;

/// Type alias for RV64I state (64-bit, 32 registers, no tracer).
pub type Rv64State = RvState<rvr_ir::Rv64, (), NUM_REGS_I>;

/// Type alias for RV32E state (32-bit, 16 registers, no tracer).
pub type Rv32EState = RvState<rvr_ir::Rv32, (), NUM_REGS_E>;

/// Type alias for RV64E state (64-bit, 16 registers, no tracer).
pub type Rv64EState = RvState<rvr_ir::Rv64, (), NUM_REGS_E>;

/// Type alias for RV32I state with tracer.
pub type Rv32StateWith<T> = RvState<rvr_ir::Rv32, T, NUM_REGS_I>;

/// Type alias for RV64I state with tracer.
...
mod tests {
    ...
    fn test_rv64_state_layout() {
        ...
        // When T = (), tracer is ZST and doesn't affect layout
        ...
        assert_eq!(offset_of!(Rv64State, target_instret), 33056);
        assert_eq!(offset_of!(Rv64State, reservation_addr), 33064);
        assert_eq!(offset_of!(Rv64State, reservation_valid), 33072);
        assert_eq!(offset_of!(Rv64State, has_exited), 33073);
        assert_eq!(offset_of!(Rv64State, exit_code), 33074);
        assert_eq!(offset_of!(Rv64State, _pad1), 33075);
        assert_eq!(offset_of!(Rv64State, _pad2), 33080);
        assert_eq!(offset_of!(Rv64State, brk), 33088);
        assert_eq!(offset_of!(Rv64State, start_brk), 33096);
        assert_eq!(size_of::<Rv64State>(), 33104);
    }

    #[test]
    fn test_rv32_state_layout() {
        // For RV32 with 32 regs, no tracer
    ...
    fn test_rv64_state_with_preflight_tracer() {
        // When tracer is PreflightTracer, it adds 32 bytes at the end
        ...
        assert_eq!(offset_of!(StateWithTracer, tracer), 33104);
    }
