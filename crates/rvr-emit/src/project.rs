...
impl<X: Xlen> CProject<X> {
    ...
    pub fn write_makefile(&self, num_partitions: usize) -> std::io::Result<()> {
        ...
        // Base flags: C23, optimization, and performance settings
        // Matches Mojo emitter for consistent codegen
        let mut cflags = vec![
            "-std=c23",
            "-O3",
            "-march=native",
            ...
            "-funroll-loops",
            "-fno-stack-protector",
            "-w", // Suppress warnings (generated code is correct)
            "-DNDEBUG",
        ];

        // LTO and related optimizations
        if self.enable_lto {
        ...
        if self.enable_lto {
            writeln!(content, "LDFLAGS = -flto=thin -fuse-ld=lld").unwrap();
        }
