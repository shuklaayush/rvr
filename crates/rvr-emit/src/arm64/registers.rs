...
/// - x17: cold-reg cache (reserved)
/// - x29 (fp), x30 (lr), sp: frame/link/stack
///
/// Available: x3-x16 (14) + x21-x28 (8) = 22 registers
pub const HOT_REG_SLOTS: usize = 22;
...
pub mod reserved {
    ...
    /// Cold reg cache (reserved)
    pub const COLD_CACHE: &str = "x17";
}

/// Available ARM64 registers for hot RISC-V register mapping.
/// Callee-saved first (x21-x28), then caller-saved (x3-x16).
/// Since we're the entry point, we save all in prologue anyway.
pub const AVAILABLE_REGS: [&str; 22] = [
...
pub const AVAILABLE_REGS_32: [&str; 22] = [
...
mod tests {
    ...
    fn test_max_hot_regs() {
        // All 22 available registers
        let hot: Vec<u8> = (1..=22).collect();
        let map = RegMap::new(&hot, false);
        ...
        assert_eq!(map.get(22), Some("x16"));

        // 23rd register should not be mapped
        let hot23: Vec<u8> = (1..=23).collect();
        let map23 = RegMap::new(&hot23, false);
        assert!(map23.get(23).is_none());
    }
