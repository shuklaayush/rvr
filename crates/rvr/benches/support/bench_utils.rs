...
fn parse_backend() -> Backend {
    match std::env::var("RVR_BENCH_BACKEND").as_deref() {
        Ok("x86" | "x86_64") => Backend::X86Asm,
        Ok("arm64" | "aarch64") => Backend::ARM64Asm,
        _ => Backend::C,
    }
}
...
fn compile_options(info: &BenchmarkInfo, backend: Backend) -> CompileOptions {
    ...
    if std::env::var("RVR_BENCH_PERF").is_ok() {
        options = options
            .with_perf_mode(true)
...
fn ensure_elf(project_dir: &Path, info: &BenchmarkInfo, arch: Arch) -> Result<PathBuf, String> {
