...
/// Run a riscv-tests host benchmark and parse its internal timing output.
///
/// These benchmarks use setStats() which prints `host_nanos = ...` with the
/// actual timed portion. We use that instead of wall clock to avoid measuring
/// process startup overhead. Perf counters are not available for subprocess.
