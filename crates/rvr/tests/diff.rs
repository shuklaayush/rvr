...
fn run_lockstep() -> Result<(), Failed> {
    let Some(elf_path) = diff_elf_path() else {
        return Ok(());
    };
    ...
    diff::compile_for_diff(&elf_path, &ref_dir, Backend::C, &compiler).map_err(Failed::from)?;
    diff::compile_for_diff(&elf_path, &test_dir, Backend::C, &compiler).map_err(Failed::from)?;
...
fn run_block_vs_linear() -> Result<(), Failed> {
    ...
    diff::compile_for_diff_block(&elf_path, &block_dir, Backend::C, &compiler)
        .map_err(Failed::from)?;
    diff::compile_for_diff(&elf_path, &linear_dir, Backend::C, &compiler).map_err(Failed::from)?;
    ...
    let result =
        diff::compare_block_vs_linear(&mut block_exec, &mut linear_exec, &config, Some(200));
    if let Some(div) = result.divergence {
        return Err(Failed::from(format!("divergence: {:?}", div.kind)));
    }
...
fn run_checkpoint() -> Result<(), Failed> {
    ...
    let mut ref_runner =
        Runner::load(&ref_dir, &elf_path).map_err(|e| Failed::from(format!("ref load: {e}")))?;
    let mut test_runner =
        Runner::load(&test_dir, &elf_path).map_err(|e| Failed::from(format!("test load: {e}")))?;
...
fn run_pure_c() -> Result<(), Failed> {
    ...
    let data = std::fs::read(&elf_path).map_err(|e| Failed::from(format!("read elf: {e}")))?;
...
fn run_pure_c_for<X: rvr_ir::Xlen>(
    ...
    let ref_lib =
        find_library_in_dir(ref_dir).ok_or_else(|| Failed::from("missing ref library"))?;
    let test_lib =
        find_library_in_dir(test_dir).ok_or_else(|| Failed::from("missing test library"))?;
