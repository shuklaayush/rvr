    ...
    fn decode32(&self, raw: u32, pc: X::Reg) -> Option<DecodedInstr<X>> {
        ...
        if opcode == OPCODE_OP_32 && X::VALUE == 64 {
            ...
            if funct7 == FUNCT7_ADD_UW && funct3 == 0b000 {
                return Some(DecodedInstr::new(OP_ADD_UW, pc, 4, InstrArgs::R { rd, rs1, rs2 }));
            }
        ...
        if opcode == OPCODE_OP_IMM_32 && X::VALUE == 64 {
            ...
            if funct3 == 0b001 && funct6 == FUNCT6_SLLI_UW {
                ...
                return Some(DecodedInstr::new(OP_SLLI_UW, pc, 4, InstrArgs::I { rd, rs1, imm: shamt }));
            }
        }
...
const OP_INFO_ZBA: &[OpInfo] = &[
    OpInfo { opid: OP_SH1ADD, name: "sh1add", class: OpClass::Alu, size_hint: 4 },
    OpInfo { opid: OP_SH2ADD, name: "sh2add", class: OpClass::Alu, size_hint: 4 },
    OpInfo { opid: OP_SH3ADD, name: "sh3add", class: OpClass::Alu, size_hint: 4 },
    OpInfo { opid: OP_ADD_UW, name: "add.uw", class: OpClass::Alu, size_hint: 4 },
    OpInfo { opid: OP_SH1ADD_UW, name: "sh1add.uw", class: OpClass::Alu, size_hint: 4 },
    OpInfo { opid: OP_SH2ADD_UW, name: "sh2add.uw", class: OpClass::Alu, size_hint: 4 },
    OpInfo { opid: OP_SH3ADD_UW, name: "sh3add.uw", class: OpClass::Alu, size_hint: 4 },
    OpInfo { opid: OP_SLLI_UW, name: "slli.uw", class: OpClass::Alu, size_hint: 4 },
];

/// Get mnemonic for Zba instruction.
pub fn zba_mnemonic(opid: OpId) -> Option<&'static str> {
    OP_INFO_ZBA.iter().find(|info| info.opid == opid).map(|info| info.name)
}
...
mod tests {
    ...
    fn test_sh1add_decode() {
        ...
        assert_eq!(instr.args, InstrArgs::R { rd: 1, rs1: 2, rs2: 3 });
    }
